...
参数全集：`--domain` · `--dynu-key`（或 `--dynu-client-id`+`--dynu-secret`）· `--email` · `--ss-port` · `--anytls-port` · `--naive-port` · `--panel-port` · `--panel-user` · `--disguise proxy:https://soft.xiaoz.org` · `--non-interactive` · `--docker`
...
- **域名伪装**：直访域名反代到伪装站（默认 `soft.xiaoz.org`），比静态页更像真实站点；naive 认证流量走隐蔽隧道
