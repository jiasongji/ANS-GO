...
|----|----|
| IPv4 | `<落地服务器IP>` |
| IPv6 | `2602:fce1:4c8:224c::`（注：宿主防火墙拦截入站，仅 IPv4 可用）|
| 系统 | Debian 12 (bookworm) / Proxmox LXC 容器 |
| 规格 | 1 vCPU / 256MB RAM + 256MB swap / 3.86GB disk |
| 域名 | `your-domain.com`（已 A + AAAA 解析到本机）|
| DNS 服务商 | Dynu Systems（NS = ns0~6.dynu.com，zone id = 14984841）|
| SSH | `root@<落地服务器IP>`（密码见 `.secrets.local`）|
...
```
            Let's Encrypt (acme.sh + Dynu DNS-01)
                        │ 签发 / 60天自动续期 + reload
                        ▼
          /etc/ssl/bv-la/{fullchain,privkey}.pem
                        │ 三服务共享引用
      ┌─────────────────┼─────────────────────┐
      ▼                 ▼                     ▼
...
3. **离线兜底**：`bv-admin` bash 脚本不依赖面板，面板全挂也能 SSH 管理一切。
...
| Web 面板 (bv-panel) | `15608` TCP | HTTPS | ✅（改后面板重启）|
...
两套都已实测可用（HTTP 200，能读写 zone 14984841）：
...
### 证书落点与续期
```
/etc/ssl/bv-la/fullchain.pem   # 证书链
/etc/ssl/bv-la/privkey.pem     # 私钥
```
- 续期周期：acme.sh 默认 60 天（实际由 ARI 窗口驱动，约 60 天）
- 续期 reload：统一走 `bv-cert-reload` 脚本（`--install-cert --reloadcmd "/usr/local/bin/bv-cert-reload"`）。该脚本按需重载——只有**当前配置引用了 `/etc/ssl/bv-la/` 证书**的服务才重载，签名发阶段是 no-op，切换证书后才会生效
- ⚠️ **caddy 用 restart 不用 reload**：Caddyfile 设了 `admin off`，无 admin API 通道，`systemctl reload caddy` 会失败。续期/改配置统一 `systemctl restart caddy`（naive 闪断 1-2s 可接受）。sing-box（ss+anytls）和 bv-panel 用 restart
...
- 伪装站：`/var/www/html/index.html`（Bing 伪装页）
...
### 5.4 客户端连接参数（部署完成后填充）
> 占位，部署脚本会自动生成并写入 §10 和 `/etc/proxy-secrets.env`
...
- 配置：`/etc/bv-panel/config.json`；会话+锁定：SQLite `/etc/bv-panel/sessions.db`（CGO-free，纯 Go 驱动 modernc.org/sqlite 避免 libc 依赖）
- systemd unit：`/etc/systemd/system/bv-panel.service`
...
| 会话有效期 | 8 小时 | ✅ 面板内改 |
| 忘记密码 | Web「忘记密码？」页显示 `SSH 执行: bv-admin panel-pass` 提示；命令打印新密码 | — |
...
4. `systemctl restart bv-panel`（当前会话断开是必然的，因为端口换了）
...
位置：`/usr/local/bin/bv-admin`（bash，零依赖，面板全挂也能用）

```bash
bv-admin status              # 三协议+面板状态一览
bv-admin info                # 打印连接参数 + URI
bv-admin restart [ss|anytls|naive|panel|all]
bv-admin stop [服务]
bv-admin logs [服务]          # tail journalctl
bv-admin regen [ss|anytls|naive]   # 重置密钥（提示确认）
bv-admin cert status         # 证书到期
bv-admin cert renew          # 手动续期
bv-admin panel-pass          # 重置面板密码（打印新密码）
bv-admin panel-path          # 重置面板 URL 路径（兜底）
bv-admin panel-port          # 重置面板端口（兜底）
bv-admin firewall [list|open PORT|close PORT]
bv-admin update [sing-box|caddy|panel]   # 升级二进制
bv-admin backup              # 备份所有配置到 /etc/bv-la-backup-{ts}/
bv-admin restore [备份目录]   # 回滚
bv-admin uninstall           # 卸载（保留配置备份）
```
...
/etc/proxy-secrets.env      # 所有协议密钥（root 独占 600）
...
| 3 | 签发 `your-domain.com` ECDSA 证书到 `/etc/ssl/bv-la/`，配 60 天自动续期 + reloadcmd | 中（DNS 传播 30-90s）| 无 |
...
| 6 | 装 `bv-admin` 脚本 | 低 | 无 |
| 7 | mac 本地交叉编译 `bv-panel`（GOOS=linux GOARCH=amd64），scp 上传 | 低 | 无 |
...
- **改配置前必备份**：`bv-admin backup` 或手工 `cp -a` 到 `/etc/bv-la-backup-{ts}/`，失败可 `bv-admin restore`。
...
> 完整凭证见服务器 `/etc/proxy-secrets.env`、`/etc/bv-panel/config.json`，
...
```
> 一键获取完整 URI：服务器执行 `bv-admin info`，或面板「节点信息」页。

### Web 面板访问
```
URL:      https://your-domain.com:15608/a32ad467/
用户名:   ad_admin
密码:     （部署时一次性显示，存 .secrets.local；遗忘用 `bv-admin panel-pass` 重置）
URL路径:  /a32ad467/  （面板内可改；遗忘用 `bv-admin panel-path` 重置）
```
...
续期后:   bv-cert-reload 自动 restart caddy/sing-box/bv-panel
```

### 服务器文件清单（与 §8 对照，均已落地）
```
/usr/local/bin/{sing-box, caddy, bv-admin, bv-genconf, bv-panel, bv-cert-reload}
/etc/bv-panel/{config.json, sessions.db}
/etc/ssl/bv-la/{fullchain.pem, privkey.pem}
...
/etc/systemd/system/{sing-box, caddy, bv-panel}.service
/etc/proxy-secrets.env  /etc/sysctl.d/99-proxy-tune.conf  /etc/security/limits.d/99-proxy.conf
```
...
| 改配置导致服务起不来 | 每次改动前 `bv-admin backup` 到 `/etc/bv-la-backup-{ts}/`，`bv-admin restore` 一键回滚 |
| 面板 Go 二进制崩溃 | systemd `Restart=on-failure` 自动重启；`bv-admin` 兜底 |
| 改面板端口后失联 | SSH 进去 `bv-admin panel-port` 重置；或改 config.json 后 restart |
| API Key 泄露 | 只存服务器 root 独占文件；可 `bv-admin` 旋转（重新填 Dynu 凭证）|
...
| 续期 reload 失败 | caddy `admin off` 无法 reload；`bv-cert-reload` 已改用 restart，续期闪断 1-2s |

---

## 12. 后续流程

1. ✅ **自测审计（已完成）**：`bv-admin status` + 面板全功能 + 三协议从中国大陆公网连通 + IP 锁定机制 + 证书真实性（Let's Encrypt）均验证通过
2. ✅ **GitHub 建项（本次完成）**：公开仓库，含 AGENTS.md + `deploy/`（脚本 + 面板源码），**不含** `.secrets.local`/`.build`
3. **服务器复现（待验证）**：用 `deploy/` 在干净服务器一键部署并测试
