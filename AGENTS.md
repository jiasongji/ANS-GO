...
>
> **当前版本：v1.5.24**（install.sh 脚本版本 v1.5.24；**面板 Go 二进制 v1.5.24 + release 资产 + ghcr.io 镜像齐全**）。完整发布历史见 GitHub Releases。
>
...
3. ✅ **服务器部署（已完成 + 持续迭代）**：面板版本迭代走 §9「stop→rm→scp→md5→start」流程（裸金属）或 `docker compose up -d`（Docker），当前 v1.5.24
