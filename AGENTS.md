...
curl -fsSL https://raw.githubusercontent.com/jiasongji/ANS-GO/main/install.sh \
  | bash -s -- --domain your-domain.com \
             ...
             --non-interactive
```
常用参数：`--domain` `--dynu-key`（或 `--dynu-client-id`+`--dynu-secret`）`--email` `--ss-port` `--anytls-port` `--naive-port` `--panel-port` `--panel-user` `--disguise-panel proxy:https://example.com` `--disguise-naive proxy:https://example.com` `--non-interactive` `--docker`（用 ghcr.io 镜像跑面板）。
