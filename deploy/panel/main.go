...
const (
	confPath    = "/etc/bv-panel/config.json"
	secretsPath = "/etc/proxy-secrets.env"
	defCertDir  = "/etc/ssl/bv-la"
)
...
func loadConfig() (Config, error) {
	...
	if c.DBPath == "" {
		c.DBPath = "/etc/bv-panel/sessions.db"
	}
...
func scheduleSelfRestart(seconds int) {
	cmd := exec.Command("sh", "-c", fmt.Sprintf("sleep %d; systemctl restart bv-panel", seconds))
...
func main() {
	// CLI 子模式: -setpass <明文>  -> bcrypt 写入 config 并打印（供 bv-admin panel-pass 调用）
	...
	if c.AdminPassHash == "" || strings.HasPrefix(c.AdminPassHash, "PLACEHOLDER") {
		log.Println("警告: 管理员密码尚未设置（PLACEHOLDER），请执行 bv-admin panel-pass 设置。")
	}
	...
	log.Printf("bv-panel v%s 监听 https://%s%s (域名 %s)", version, addr, c.URLPath, c.Domain)
...
func cliSetPass(plain string) error {
	...
	if err != nil {
		// 配置缺失时用零值继续（允许首次设置）
		c = Config{PanelPort: 15608, SessionHours: 8, LoginLockThreshold: 5, LoginLockMinutes: 10, CertDir: defCertDir, DBPath: "/etc/bv-panel/sessions.db"}
	}
