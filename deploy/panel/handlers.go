...
func buildURIs(c Config, s secretData) map[string]string {
	...
	if s.SSKey != "" {
		...
		u["ss"] = fmt.Sprintf("ss://%s@%s:%d#BV-LA-SS", ui, c.Domain, c.SSPort)
	}
	if s.AnyTLSPass != "" {
		u["anytls"] = fmt.Sprintf("anytls://%s@%s:%d/?sni=%s#BV-LA-AnyTLS",
			s.AnyTLSPass, c.Domain, c.AnyTLSPort, c.Domain)
	}
	if s.NaiveUser != "" {
		u["naive"] = fmt.Sprintf("naive+https://%s:%s@%s:%d#BV-LA-Naive",
