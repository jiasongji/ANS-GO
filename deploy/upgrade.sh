#!/usr/bin/env bash
# =============================================================================
# ANS-GO 一键升级脚本 (upgrade.sh)   v1.5.16
#
...
#   bash upgrade.sh [--docker | --metal] [--ver v1.5.16] [--yes]
#
...
#     启动后用 journalctl 日志行 "ansgo-panel v1.5.16 监听..." 验证版本
...
升级内容（${VER}）:
  - SOCKS5 支持（sing-box 第三个 inbound，强制用户名/密码鉴权）
  - 面板自定义网页标题
  - NaiveProxy 落地简化（落地仅 AnyTLS-2）
