#!/bin/bash
# =============================================================================
# bv-deploy-cert.sh —— AGENTS §9 步骤 1-3（服务端执行）
#   1. 安装 acme.sh（curl，~200KB）
#   2. 部署 dns_dynukey.sh（路径A 钩子）+ 校验路径B 凭证可达
#   3. 签发 your-domain.com ECDSA 证书：路径A(默认) → 失败降级路径B
#      安装到 /etc/ssl/bv-la/{fullchain,privkey}.pem，reloadcmd=bv-cert-reload
#
# 用法（凭证经环境传入，不落盘额外文件）：
#   DYNU_API_KEY=... DYNU_CLIENT_ID=... DYNU_SECRET=... \
#     nohup bash bv-deploy-cert.sh > /root/bv-deploy-cert.log 2>&1 &
#
# 输出状态文件：/etc/bv-la-cert.status  (SUCCESS_A | SUCCESS_B | FAILED)
# =============================================================================
set -uo pipefail
DOMAIN="your-domain.com"
EMAIL="admin@${DOMAIN}"
CERTDIR="/etc/ssl/bv-la"
STATUS_FILE="/etc/bv-la-cert.status"
...
# ---- 部署重载助手 + 钩子 ----
install -m 0755 /etc/bv-deploy/bv-cert-reload /usr/local/bin/bv-cert-reload
mkdir -p /root/.acme.sh/dnsapi
install -m 0644 /etc/bv-deploy/dns_dynukey.sh /root/.acme.sh/dnsapi/dns_dynukey.sh
log "已部署 bv-cert-reload 与 dns_dynukey.sh 钩子。"
...
log "=== 安装证书到 ${CERTDIR}（reloadcmd=bv-cert-reload）==="
if "$ACME" --install-cert -d "$DOMAIN" --ecc \
    --key-file       "${CERTDIR}/privkey.pem" \
    --fullchain-file "${CERTDIR}/fullchain.pem" \
    --reloadcmd      "/usr/local/bin/bv-cert-reload"; then
  log "证书安装完成。"
else
